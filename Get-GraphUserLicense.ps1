...
function GetProductInfo() {
    ...
    $m365licenseInfoCsvPath = Join-Path $licenseReportPath 'M365LicenseInfo.csv'
    ...
    $m365licenseInfoCsvPath = Import-Csv $m365licenseInfoCsvPath

    foreach ($line in $m365licenseInfoCsv) {
...
function GetLicenseTypesInTenant {
    ...
    $licenseTypeSummaryReportPath = Join-Path $licenseReportPath ($date + '__LicenseSummary.csv')
    ...
    foreach ($item in $licenseTypes) {
        $itemInfo = [ordered]@{
            ...
            Name          = GetProductDisplayName $item.SkuId
            Total         = $item.Enabled
...
function GetProductDisplayName($skuId) {
...
do {
    ...
    else 
    {
        ...
        Write-Host $licensedUsers.value.Count
    }
    ...
    foreach ($licenseType in $licenseTypes) {    
        ...
        $productDisplayName = GetProductDisplayName $licenseType.SkuId
